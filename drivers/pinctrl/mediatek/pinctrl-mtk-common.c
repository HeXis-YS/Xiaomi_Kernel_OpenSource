...
static void mtk_gpio_set(struct gpio_chip *chip, unsigned int offset, int value)
{
	unsigned int reg_addr;
	unsigned int bit;
				      ...
				      unsigned int type)
{
	...
	if (((type & IRQ_TYPE_EDGE_BOTH) && (type & IRQ_TYPE_LEVEL_MASK)) ||
		...
		pr_info("[GPIO]Can't configure IRQ%d (EINT%lu) for type 0x%X\n",
			d->irq, d->hwirq, type);
		return -EINVAL;
	}
		...
		struct regmap *regmap)
{
	...
	if (data->regmap_num == 0) {
		...
		if (node) {
			...
			pr_warn("Pinctrl node has not register regmap\n");
			return -EINVAL;
		}
	...
	pctl->pctl_dev = pinctrl_register(&pctl->pctl_desc, &pdev->dev,
					       pctl);
